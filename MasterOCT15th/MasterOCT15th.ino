...
void JoystickEvents::OnGamePadChanged(const uint8_t* data, uint8_t len) {
  // For debugging, print out the data
  Serial.print(F("Data: "));
  ...
  // Adjust indices based on your joystick's report format
  uint8_t xValue = data[0]; // Assuming X-axis is at data[0]
  uint8_t yValue = data[1]; // Assuming Y-axis is at data[1]
